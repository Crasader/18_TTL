...
bool XZDDGameScence::StartRecord(datastream kDataStream)
{
	...
	{
		m_pGameRecord = new XZDD::XZDDGameRecord;
		initRecord();
	}
	defaultState();
	...
	*m_pGameRecord = XZDD::XZDDGameRecord();
	WidgetFun::setVisible(this,"Button_GameRecordPlay",true);
	WidgetFun::setVisible(this,"Button_GameRecordPase",false);
	m_iActRecordIdex = 0;
	m_RecordTime = 2.0f;
	...
	{
		XZDD::XZDDGameRecordPlayer& kRecordPlayer = m_pGameRecord->kPlayers[i];
...
void XZDDGameScence::NextRecordAction()
{
	...
	XZDD::XZDDGameRecordOperateResult& kAction = m_pGameRecord->kAction[m_iActRecordIdex];
	int iChairID = (m_wRecordSelfChairID-kAction.wOperateUser+MAX_PLAYER)%MAX_PLAYER;
	if (kAction.cbActionType == XZDD::XZDDGameRecordOperateResult::TYPE_OperateResult)
	{
		XZDDPlayer* pPlayer = m_pPlayer[iChairID];
		XZDD::CMD_S_OperateResult kTempCMD;
	...
	if (kAction.cbActionType == XZDD::XZDDGameRecordOperateResult::TYPE_SendCard)
	{
		XZDDPlayer* pPlayer = m_pPlayer[iChairID];
	...
	if (kAction.cbActionType == XZDD::XZDDGameRecordOperateResult::TYPE_OutCard)
	{
		XZDDPlayer* pPlayer = m_pPlayer[iChairID];
	...
	if (kAction.cbActionType == XZDD::XZDDGameRecordOperateResult::TYPE_ChiHu)
	{
		XZDDPlayer* pPlayer = m_pPlayer[iChairID];
		for (int i = 0;i<MAX_PLAYER;i++)
		{
		...
		else
		{
			...
			pPlayer->runAniZiMo();
			//É¾³ýÃþµÄÅÆ
			pPlayer->setshanchuCard(kAction.cbOperateCard);
