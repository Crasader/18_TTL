...
void XZDDGameScence::setPrivateInfo(CMD_GF_Private_Room_Info* pNetInfo)//设置房间私有信息
{
	...
	std::string temp = FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_HUAN3_))?"血战换三张":"血战麻将";
	if(temp != "")
	vRuleNam.push_back(temp);
	temp = FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_GUOSHOUHU_))?"过手胡":"过小胡大";
	if(temp != "")
	vRuleNam.push_back(temp);
	temp = FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_D_G_H_ZIMO_))?"点杠花(自摸)":"点杠花(放炮)";
	if(temp != "")
	vRuleNam.push_back(temp);
	temp = FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_ZI_MO_JIADI_))?"自摸加底":"自摸加番";
	if(temp != "")
	vRuleNam.push_back(temp);
	//ruleNam += FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_ZI_MO_JIAFAN_))?"自摸加番\n":"";
	temp = FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_HAI_DI_LAO_))?"海底捞":"";
	if(temp != "")
	vRuleNam.push_back(temp);
	temp = FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_MEN_Q_))?"门清":"";
	if(temp != "")
	vRuleNam.push_back(temp);
	temp = FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_ZHONG_ZHANG_))?"中张":"";
	if(temp != "")
	vRuleNam.push_back(temp);
	temp = FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_YI_JIU_))?"幺九":"";
	if(temp != "")
	vRuleNam.push_back(temp);
	temp = FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_Jiang_dui_))?"将对":"";
	if(temp != "")
	vRuleNam.push_back(temp);
	temp = "4番";
	vRuleNam.push_back(temp);
...
void XZDDGameScence::friendPrivateInfo()//设置房间私有信息
{
	...
	resultRule +=FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_HUAN3_))?"血战换三张":"血战麻将";
	resultRule +=FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_GUOSHOUHU_))?",过手胡":",过小胡大";
	resultRule +=FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_D_G_H_ZIMO_))?",点杠花(自摸)":",点杠花(放炮)";
	resultRule +=FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_ZI_MO_JIADI_))?",自摸加底":",自摸加番";
	//ruleNam += FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_ZI_MO_JIAFAN_))?"自摸加番\n":"";
	resultRule +=FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_HAI_DI_LAO_))?",海底捞":"";
	resultRule +=FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_MEN_Q_))?",门清":"";
	resultRule +=FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_ZHONG_ZHANG_))?",中张":"";
	resultRule +=FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_YI_JIU_))?",幺九":"";
	resultRule +=FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_Jiang_dui_))?",将对":"";
	resultRule +=",4番";
	resultRule +=utility::toString(",底注：", m_kPrivateRoomInfo.dwBaseScore);
	std::string resultTitle =FvMask::HasAll(m_kPrivateRoomInfo.bGameRuleIdex, _MASK_(GPHomeCreateRoomPanel_XZDD::GAME_RULE_HUAN3_))?"血战换三张":"血战麻将";
