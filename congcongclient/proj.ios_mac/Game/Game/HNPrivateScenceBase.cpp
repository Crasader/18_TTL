...
void HNPrivateScenceBase::ConnectAndJoinPrivateByRoomNum(std::string roomNum)
{
    if(roomNum.length() == 6) {
        memset(m_kJoinNumTxt, 0, sizeof(m_kJoinNumTxt));
        memcpy(m_kJoinNumTxt, roomNum.c_str(), utility::Min(sizeof(roomNum), sizeof(m_kJoinNumTxt)));
        m_kActJoinNum = utility::parseInt(roomNum);
        int iServerID = m_kActJoinNum / 10000 - 10;
        CGameServerItem* pGameServer = GameManagerBase::InstanceBase().SearchGameServer(iServerID);
