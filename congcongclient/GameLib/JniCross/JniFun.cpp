...
namespace JniFun
{
...
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
        JniMethodInfo minfo;
        bool isHave = JniHelper::getStaticMethodInfo(minfo,JAVA_CLASSNAME,"onLoginError","(I)V");
        if (isHave)
        {
            minfo.env->CallStaticVoidMethod(minfo.classID, minfo.methodID, errorCode);
	...
	void longinWX(const char* APP_ID, const char* AppSecret)
	{
		...
		if (isHave)
		{
			...
			minfo.env->DeleteLocalRef(minfo.classID);
			cocos2d::log("Jni call longinWX");
		}
		else
		{
			cocos2d::log("Jni call longinWX error!");
		}
#endif
#if CC_TARGET_PLATFORM == CC_PLATFORM_IOS
		IosHelper::sendAuthRequest();
	...
	void shareTextWX(const char* kText,int nType)
	{
			...
			if (nType == 0)
			{
				IosHelper::shareWithWeixinFriendTxt2(kText);
			}
			else
			{
	...
	void shareUrlWX(const char* kUrl,const char* kTitle,const char* kDesc,int nType)
	{
		...
		if (nType == 0)
		{
			IosHelper::shareWithWeixinFriendTxt(kTitle,kUrl);
		}
		else
		{
