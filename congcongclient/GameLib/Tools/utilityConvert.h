...
/************************************************************************/
#ifndef _UTILITY_CONVERT_H_
#define _UTILITY_CONVERT_H_
...
namespace utility
{
	 ...
	 * @return 字符串长度
	 */
	inline int utf8_len(const char *utf8)
	{
		int i = 0, j = 0;
		while (utf8[i]) {
	...
	inline int utf8_cmp(const char* str1, const char* str2)
	{
	 ...
	 * @return 字符串长度
	 */
	inline int ucs2_len(const unsigned short* ucs2)
	{
		int i = 0;
		while (ucs2[i] != 0)
		{
	...
	/////////////////////////////////////////////////////////////////////////////
	inline int code_convert(const char *from_charset, const char *to_charset, const char *inbuf, size_t inlen, char *outbuf, size_t outlen)
	{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
		iconv_t cd;
	 ...
	 * @return 字符串长度
	 */
	inline int utf8_len(std::string utf8)
	{
		return strlen(utf8.c_str());
	}
	inline int IsTextUTF8(const char* str,unsigned int  length)
	{  
		return 0;
		unsigned int i;  
	...
	inline std::string a_u8(std::string gbk)
	{
	...
	inline std::string u8_a(std::string utf8)
	{
