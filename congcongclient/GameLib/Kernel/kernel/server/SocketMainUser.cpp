...
bool CServerItem::OnSocketSubUserEnter(void* data, int dataSize)
{
	...
	//人数更新
	pSevList->SetServerOnLineCount(mServerAttribute.wServerID,mUserManager->GetActiveUserCount());
...
bool CServerItem::OnSocketSubUserStatus(void* data, int dataSize)
{
	...
	{
		//用户自己离开了桌子
		if (pIClientUserItem->GetUserID() == UserInfo::Instance().getUserID() && UserStatus.wTableID == INVALID_TABLE) {

			mUserManager->DeleteAllUserItemButOne(pIClientUserItem);
