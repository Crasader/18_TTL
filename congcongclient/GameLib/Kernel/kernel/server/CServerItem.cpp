...
bool CServerItem::ConnectServer(CGameServerItem * pGameServerItem,word wAVServerPort,dword dwAVServerAddr)
{
	...
	{
		if (mIStringMessageSink)
			mIStringMessageSink->InsertPromptString(("System_Tips_3"), DLG_MB_OK);
		return false;
	}
...
bool CServerItem::sendSubGRPrivateInfo()
{
	CMD_GR_Private_Info_Server cmd_subGRPrivateInfo;
	return SendSocketData(MDM_GR_PRIVATE, SUB_GR_PRIVATE_INFO, &cmd_subGRPrivateInfo, sizeof(CMD_GR_Private_Info_Server));
}

bool CServerItem::sendPrivateTablesInfo()
{
	CMD_GR_INQURE_TABLES_INFO cmd_inqure_table;
	cmd_inqure_table.bGetTableInfo = true;
	return SendSocketData(MDM_GR_PRIVATE, SUB_GR_INQUIRE_TABLES, &cmd_inqure_table, sizeof(CMD_GR_INQURE_TABLES_INFO));
}
...
bool CServerItem::onEventTCPHeartTick()
{
	...
	//SendSocketData(MDM_KN_COMMAND, SUB_KN_DETECT_SOCKET, pCommand.pDataBuffer, pCommand.wDataSize);

#endif
	return true;
}

bool CServerItem::OnSocketSystemMessage(int sub, void* data, int dataSize)
{
    switch(sub)
    {
        case SUB_C2L_SYSTEM_NOTIFY:
        {
            return OnSocketSystemNotify(sub, data, dataSize);
        }
    }
...
bool CServerItem::OnSocketSystemNotify(int sub, void* data, int dataSize)
{
	...
	//cocos2d::UserDefault::sharedUserDefault()->setStringForKey("NoticeMsg",pGameSystem->szInfo);
    //int msgindex = cocos2d::UserDefault::sharedUserDefault()->getIntegerForKey("NoticeMsgIndex",0);
	//cocos2d::UserDefault::sharedUserDefault()->setIntegerForKey("NoticeMsgIndex",msgindex+1);
	return true;
}

int CServerItem::GetCurServerChairsCount()
{
	return m_TableFrame.GetChairCount();
}
