...
void CGMatchMission::PerformSignupMatch(int iGamerServerID)
{
	...
	//设置变量
	CGameServerItem * pServerItem = pSevList->SearchGameServer(iGamerServerID);
	if (pServerItem)
	{
		//设置变量
		m_bSignup=true;
...
void CGMatchMission::Net_SignupResult(void* data, int dataSize)
{
	...
	//查找房间
	CGameServerItem * pGameServerItem = pSevList->SearchGameServer(m_pGameMatchInfo.wServerID);
