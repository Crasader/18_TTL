...
#define SUB_GR_USER_SHARED_WEIXIN		12									//用户微信分享成功
...
struct CMD_GR_USER_SHARE_WEIXIN
{
	dword							dwUserID;							//用户标识
	dword							dwShareType;						//分享的方式
};
...
#define SUB_GR_INQUIRE_TABLES                                         409 //查看私人场所有桌子信息
#define SUB_GR_INQUIRE_TABLES_RET                                  410 //服务器返回查询私人场桌子结果
#define SUB_GR_REQUEST_REFRESH_TABLE                           411 //私人场通知刷新桌子信息

//私人场信息
struct CMD_GR_Private_Info_Server
{
	word								wKindID;
	SCORE							lCostGold;
...
{	
	...
	CMD_GF_Private_Room_Info& operator=(CMD_GF_Private_Room_Info& val) {
		...
		this->kWinLoseScore.resize(val.kWinLoseScore.size());
		for (size_t i = 0; i < val.kWinLoseScore.size(); i++) {
			this->kWinLoseScore[i] = val.kWinLoseScore[i];
		}
		return *this;
	}
...
struct CMD_GR_INQURE_TABLES_INFO
{
    dword   bGetTableInfo;
};

struct CMD_GR_INQURE_TABLES_INFO_DATA_HEAD
{
    dword                   dwGameKindID;
    dword                   dwTablesTotal;
};
...
//////////////////////////////////////////////////////////////////////////////////
//抽奖测试信息
#define MDM_GR_CHOUJIANG				11									//抽奖测试信息

#define SUB_GR_CHOUJIANG_INFO			401								    //抽奖信息
#define SUB_GR_CHOUJIANG_FAIL			402									//抽奖失败
#define SUB_GR_CHOUJIANG_SUCCESS    	403									//抽奖成功

struct CMD_GF_ChouJiang_Info
{	
	CMD_GF_ChouJiang_Info()
	{
		zeromemory(this,sizeof(CMD_GF_ChouJiang_Info));
	}
	dword			dwUserId;
};

struct CMD_GF_ChouJiang_Fail
{	
	CMD_GF_ChouJiang_Fail()
	{
		zeromemory(this,sizeof(CMD_GF_ChouJiang_Fail));
	}
	bool			isFail;
	dword			dwUserId;
};

struct CMD_GF_ChouJiang_Success 
{	
	CMD_GF_ChouJiang_Success()
	{
		zeromemory(this,sizeof(CMD_GF_ChouJiang_Success));
	}
	bool			isSuccess;
	dword			dwUserId;
};
...
#define     MDM_C2G_SYSTEM_MSG    300           //系统消息

#define     SUB_C2L_SYSTEM_NOTIFY   100          //系统通知

//////////////////////////////////////////////////////////////////////////

#define SYS_MSG_LENGTH  1024*5

//系统通知
struct CMD_MB_System_Notify_C
{
    byte bFlag;
    char szInfo[SYS_MSG_LENGTH];

    void clone(char* buffer)
    {
        size_t len = strlen(buffer);
        memcpy(szInfo, szInfo, len);
        szInfo[len] = 0;
    }

    size_t size()
    {
        return sizeof(bFlag) + strlen(szInfo) + 1;
    }
