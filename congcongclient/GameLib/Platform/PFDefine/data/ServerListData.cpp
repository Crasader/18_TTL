...
FV_SINGLETON_STORAGE(CServerListData)
...
CGameServerItem*  CServerListData::getGameServerByKind(word wKindID,word wServerType)
{
	...
	CGameServerItemMap::iterator itor = GetServerItemMapBegin();
	CGameServerItem* pListItem = EmunGameServerItem(itor);
	while (pListItem != NULL)
	{
		CGameServerItem* pActListItem = pListItem;
		pListItem = EmunGameServerItem(itor);
...
void CServerListData::OnEventKindFinish(uint16 wKindID)
{
	...
	ASSERT(m_pIServerListDataSink!=0);
	if (m_pIServerListDataSink!=0)
		m_pIServerListDataSink->OnGameKindFinish(wKindID);
...
void CServerListData::SetServerOnLineFinish()
{
	...
	ASSERT(m_pIServerListDataSink!=0);
	if (m_pIServerListDataSink!=0)
		m_pIServerListDataSink->OnGameItemUpdateFinish();
}
