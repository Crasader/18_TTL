...
#define pSevList CServerListData::pInstance()

//列表服务
class CServerListData : public FvSingleton<CServerListData>
{
	FRIEND_SINGLETON(CServerListData)
protected:
...
public:
	CGameServerItem* getGameServerByKind(word wKindID,word wServerType);
...
public:
	//种类数目
	uint32 GetGameTypeCount() {
		return (uint32)m_GameTypeItemMap.size();
	}
	//类型数目
	uint32 GetGameKindCount() {
		return (uint32)m_GameKindItemMap.size();
	}
	//房间数目
	uint32 GetGameServerCount() {
		return (uint32)m_GameServerItemMap.size();
	}
	//获取总在线人数
