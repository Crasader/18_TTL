...
void WidgetManager::changeCheckBoxProperty(cocos2d::Node* pNode,std::string kKey,std::string kValue)
{
	...
	if (kKey == "Enabled") {
		pCheckBox->setTouchEnabled(utility::parseBool(kValue));
	}
}
...
void WidgetManager::changeSelectBoxProperty( cocos2d::Node* pNode,std::string kKey,std::string kValue )
{
	...
	if (pSwitch){
		if (kKey == "UnSelectTexture") {
			pSwitch->setUnSelectFileName(kValue);
		}
		if (kKey == "SelectTexture") {
			pSwitch->setSelectFileName(kValue);
		}
		if (kKey == "Checked") {
			pSwitch->setSelectStatus(utility::parseBool(kValue));
		}
		if (kKey == "Enable") {
			pSwitch->setTouchEnabled(utility::parseBool(kValue));
		}
	}
}
