...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBO_GP_GAME_RECORD_LIST_EX:		//商城列表
	{
		return OnDBPCGameRecordListEx(dwContextID, pData, wDataSize);
	}
	case DBO_GP_GAME_RECORD_TOTAL:		//商城列表
		{
			return OnDBPCGameRecordTotal(dwContextID,pData,wDataSize);
		}
	case DBO_GP_GAME_RECORD_CHILD:		//商城列表
		{
			return OnDBPCGameRecordChild(dwContextID,pData,wDataSize);
		}
	case DBO_GP_POPULARIZE_LIST: {
		return OnDBPopularizeList(dwContextID, pData, wDataSize);
	}
	case DBO_GP_POPULARIZE_BIND: {
		return OnDBPopularizeBind(dwContextID, pData, wDataSize);
		}
	case DBO_GP_POPULARIZE_HTTP:{
			return OnDBPopularizeHttp(dwContextID, pData, wDataSize);
		}
	}
...
bool CAttemperEngineSink::OnTCPNetworkMainPCUserService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	switch (wSubCmdID)
	{
	case SUB_GP_POPULARIZE_LIST: {
		//效验参数
		ASSERT(wDataSize == sizeof(CMD_GP_PopularizeList));
		if (wDataSize != sizeof(CMD_GP_PopularizeList)) return false;

		//处理消息
		CMD_GP_PopularizeList * pNetInfo = (CMD_GP_PopularizeList *)pData;
		//变量定义
		DBR_GP_PopularizeList kDBInfo;
		ZeroMemory(&kDBInfo, sizeof(kDBInfo));

		//构造数据
		kDBInfo.bPopularizeType = pNetInfo->bPopularizeType;
		kDBInfo.dwGameKindID = pNetInfo->dwGameKindID;

		//投递请求
		m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_POPULARIZE_LIST, dwSocketID, &kDBInfo, sizeof(kDBInfo));
		return true;
	}
	case SUB_GP_POPULARIZE_BIND: {
		//效验参数
		ASSERT(wDataSize == sizeof(CMD_GP_PopularizeBind));
		if (wDataSize != sizeof(CMD_GP_PopularizeBind)) return false;

		//处理消息
		CMD_GP_PopularizeBind * pNetInfo = (CMD_GP_PopularizeBind *)pData;
		//变量定义
		DBR_GP_PopularizeBind kDBInfo;
		ZeroMemory(&kDBInfo, sizeof(kDBInfo));

		//构造数据
		kDBInfo.dwOperatorUser = pNetInfo->dwOperatorUser;
		memcpy(kDBInfo.szPopularizeCode, pNetInfo->szPopularizeCode, sizeof(kDBInfo.szPopularizeCode));

		//投递请求
		m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_POPULARIZE_BIND, dwSocketID, &kDBInfo, sizeof(kDBInfo));

		return true;
	}
	case SUB_GP_POPULARIZE_HTTP:
		{
			//效验参数
			ASSERT(wDataSize == sizeof(CMD_GP_PopularizeHttp));
			if (wDataSize != sizeof(CMD_GP_PopularizeHttp)) return false;

			//处理消息
			CMD_GP_PopularizeHttp * pNetInfo = (CMD_GP_PopularizeHttp *)pData;
			//变量定义
			DBR_GP_PopularizeHttp kDBInfo;
			ZeroMemory(&kDBInfo, sizeof(kDBInfo));

			//构造数据
			kDBInfo.dwVersion = pNetInfo->dwVersion;
			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_POPULARIZE_HTTP, dwSocketID, &kDBInfo, sizeof(kDBInfo));
	...
	case SUB_GP_GAME_RECORD_LIST_EX: {
		//效验参数
		ASSERT(wDataSize == sizeof(CMD_GP_GetGameRecord_List));
		if (wDataSize != sizeof(CMD_GP_GetGameRecord_List)) return false;
		...
		CMD_GP_GetGameRecord_List * pNetInfo = (CMD_GP_GetGameRecord_List *)pData;
		//变量定义
		DBR_GP_GameRecordListEx kDBInfo;
		ZeroMemory(&kDBInfo, sizeof(kDBInfo));
	...
	case SUB_GP_GAME_RECORD_CHILD://游戏记录
		{
			//效验参数
	...
	case SUB_GP_SHOPCARD:
		{
			CMD_GP_ShopCard ShopCard;
			ZeroMemory(&ShopCard,sizeof(ShopCard));

			//构造数据
			ShopCard.lResultCode=0;
			lstrcpyn(ShopCard.szDescribeString,TEXT("11111"),CountArray(ShopCard.szDescribeString));

			//发送数据
			WORD wDescribe=CountStringBuffer(ShopCard.szDescribeString);
			WORD wHeadSize=sizeof(ShopCard)-sizeof(ShopCard.szDescribeString);
			m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GP_USER_SERVICE,SUB_GP_OPERATE_FAILURE,&ShopCard,wHeadSize+wDescribe);
...
bool CAttemperEngineSink::OnDBPCLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	lstrcpyn(pCMDLogonSuccess->szGroupName,"亲！充值钻石服务请加微信ximianfang，24小时客服电话15982265753",CountArray(pCMDLogonSuccess->szGroupName));
	//用户成绩
...
//推广人列表
bool CAttemperEngineSink::OnDBPopularizeList(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//判断在线
	ASSERT(LOWORD(dwContextID) < m_pInitParameter->m_wMaxConnect);
	if ((m_pBindParameter + LOWORD(dwContextID))->dwSocketID != dwContextID) return true;

	m_pITCPNetworkEngine->SendData(dwContextID, MDM_GP_USER_SERVICE, SUB_GP_POPULARIZE_LIST, pData, wDataSize);
	m_pITCPNetworkEngine->ShutDownSocket(dwContextID);
	return true;
}

//推广人列表
bool CAttemperEngineSink::OnDBPopularizeBind(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//判断在线
	ASSERT(LOWORD(dwContextID) < m_pInitParameter->m_wMaxConnect);
	if ((m_pBindParameter + LOWORD(dwContextID))->dwSocketID != dwContextID) return true;

	m_pITCPNetworkEngine->SendData(dwContextID, MDM_GP_USER_SERVICE, SUB_GP_POPULARIZE_BIND, pData, wDataSize);
	m_pITCPNetworkEngine->ShutDownSocket(dwContextID);
	return true;
}

//推广HTTP
bool CAttemperEngineSink::OnDBPopularizeHttp(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//判断在线
	ASSERT(LOWORD(dwContextID) < m_pInitParameter->m_wMaxConnect);
	if ((m_pBindParameter + LOWORD(dwContextID))->dwSocketID != dwContextID) return true;

	m_pITCPNetworkEngine->SendData(dwContextID, MDM_GP_USER_SERVICE, SUB_GP_POPULARIZE_HTTP, pData, wDataSize);
	m_pITCPNetworkEngine->ShutDownSocket(dwContextID);
	return true;
}
...
bool CAttemperEngineSink::OnDBPCGameRecordListEx(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//判断在线
	ASSERT(LOWORD(dwContextID) < m_pInitParameter->m_wMaxConnect);
	if ((m_pBindParameter + LOWORD(dwContextID))->dwSocketID != dwContextID) return true;

	m_pITCPNetworkEngine->SendData(dwContextID, MDM_GP_USER_SERVICE, SUB_GP_GAME_RECORD_LIST_EX, pData, wDataSize);
	m_pITCPNetworkEngine->ShutDownSocket(dwContextID);
	return true;
}
