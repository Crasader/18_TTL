...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GP_POPULARIZE_LIST: {
		return OnRequestPopularizeList(dwContextID, pData, wDataSize);
	}
	case DBR_GP_POPULARIZE_BIND: {
		return OnRequestPopularizeBind(dwContextID, pData, wDataSize);
	}
	case DBR_GP_POPULARIZE_HTTP: {
		return OnRequestPopularizeHttp(dwContextID,pData,wDataSize);
	}
	case DBR_GP_GAME_RECORD_LIST:
		{
			return OnRequestGameRecordList(dwContextID,pData,wDataSize);
		}
	case DBR_GP_GAME_RECORD_LIST_EX:
	{
		return OnRequestGameRecordListEx(dwContextID, pData, wDataSize);
	}
...
bool CDataBaseEngineSink::OnRequestBeginnerQueryInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		if (m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GR_BeginnerQueryInfo"),true)==DB_SUCCESS)
		{
			...
			CheckInInfo.bTodayChecked = static_cast<bool>(m_AccountsDBAide.GetValue_BYTE(TEXT("TodayCheckIned")));
			CheckInInfo.bLastCheckIned = static_cast<bool>(m_AccountsDBAide.GetValue_BYTE(TEXT("LastCheckIned")));
		...
		if(lResultCode==DB_SUCCESS)
		{
			...
			while (m_AccountsDBModule->IsRecordsetEnd()==false)
			{
				...
				if(wDayIndex<=LEN_ADDRANK && wDayIndex >= 0)
				{
					...
					kDBInfo.kRewardGold[wDayIndex] = static_cast<int>(m_AccountsDBAide.GetValue_LONGLONG(TEXT("RewardGold")));
					kDBInfo.kRewardType[wDayIndex] = static_cast<int>(m_AccountsDBAide.GetValue_LONGLONG(TEXT("RewardType")));
				}
				//移动记录
				m_AccountsDBModule->MoveToNext();
			}
	...
	try
	{
		...
		if(lResultCode==DB_SUCCESS)
		{
			...
			while (m_AccountsDBModule->IsRecordsetEnd()==false)
			{
				...
				kDBInfo.kUserPoint.push_back(static_cast<INT>(m_AccountsDBAide.GetValue_LONGLONG(TEXT("UserPoint")))); 
...
bool CDataBaseEngineSink::OnRequestRouletteReward(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		if(lResultCode==DB_SUCCESS)
		{
			...
			kDBInfo.cbCout =  static_cast<BYTE>(wIndex);
...
bool CDataBaseEngineSink::OnRequestLoadBaseEnsure(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		...
		if(lResultCode==DB_SUCCESS)
		{
			...
			{
				BaseEnsureParameter.cbTakeTimes =  static_cast<BYTE>(m_AccountsDBAide.GetValue_WORD(TEXT("TakeTimes")));
...
//获取推广人列表
bool CDataBaseEngineSink::OnRequestPopularizeList(DWORD dwContextID, VOID * pData, WORD wDataSize) 
{
	//效验参数
	ASSERT(wDataSize == sizeof(DBR_GP_PopularizeList));
	if (wDataSize != sizeof(DBR_GP_PopularizeList)) return false;

	//变量定义
	DBR_GP_PopularizeList * pRequest = (DBR_GP_PopularizeList *)pData;

	//请求处理
	try {
		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@type"), pRequest->bPopularizeType);
		m_TreasureDBAide.AddParameter(TEXT("@size"), 50);

		//结果处理
		LONG lReturnValue = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_GetPopularizeList"), true);

		CMD_GP_PopularizeListRes popularizeList;
		if (lReturnValue == DB_SUCCESS) {
			popularizeList.dwGameKindID = pRequest->dwGameKindID;
			popularizeList.bPopularizeType = pRequest->bPopularizeType;

			while (m_TreasureDBModule->IsRecordsetEnd() == false) {
				CMD_GP_PopularizeInfo popularizeInfo;
				popularizeInfo.iPopularizeID = 0;
				popularizeInfo.iRankCount = 0;
				popularizeInfo.dwUserID = m_TreasureDBAide.GetValue_DWORD(TEXT("UserID"));
				popularizeInfo.lScore = m_TreasureDBAide.GetValue_LONGLONG(TEXT("Score"));
				m_TreasureDBAide.GetValue_String(TEXT("NickName"), popularizeInfo.szName, CountArray(popularizeInfo.szName));
				m_TreasureDBAide.GetValue_String(TEXT("HeadHttp"), popularizeInfo.szAvatar, CountArray(popularizeInfo.szAvatar));
				popularizeList.vcPopularizeList.push_back(popularizeInfo);
				m_TreasureDBModule->MoveToNext();
			}
		}

		datastream kDataStream;
		popularizeList.StreamValue(kDataStream, true);
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_POPULARIZE_LIST, dwContextID, &kDataStream[0], kDataStream.size());
		return true;
	} catch (IDataBaseException * pIException) {
		//错误信息
		CTraceService::TraceString(pIException->GetExceptionDescribe(), TraceLevel_Exception);
...
//绑定邀请码
bool CDataBaseEngineSink::OnRequestPopularizeBind(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//效验参数
	ASSERT(wDataSize == sizeof(DBR_GP_PopularizeBind));
	if (wDataSize != sizeof(DBR_GP_PopularizeBind)) return false;

	//变量定义
	DBR_GP_PopularizeBind * pRequest = (DBR_GP_PopularizeBind *)pData;

	//请求处理
	try {
		//构造参数
		m_TreasureDBAide.ResetParameter();
		m_TreasureDBAide.AddParameter(TEXT("@bindUser"), pRequest->dwOperatorUser);
		m_TreasureDBAide.AddParameter(TEXT("@popularizeCode"), pRequest->szPopularizeCode);
		//结果处理
		LONG lReturnValue = m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_PopularizeBind"), true);

		CMD_GP_PopularizeBindRes popularizeBind;
		popularizeBind.bResult = (BYTE)lReturnValue;
		m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_POPULARIZE_BIND, dwContextID, &popularizeBind, sizeof(popularizeBind));
		return true;
	} catch (IDataBaseException * pIException) {
		//错误信息
		CTraceService::TraceString(pIException->GetExceptionDescribe(), TraceLevel_Exception);
...
//绑定邀请码
bool CDataBaseEngineSink::OnRequestPopularizeHttp(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	//效验参数
	ASSERT(wDataSize == sizeof(DBR_GP_PopularizeHttp));
	if (wDataSize != sizeof(DBR_GP_PopularizeHttp)) return false;

	//变量定义
	DBR_GP_PopularizeHttp * pRequest = (DBR_GP_PopularizeHttp *)pData;
	...
	try {
		...
		m_AccountsDBAide.ResetParameter();
		//结果处理
		LONG lReturnValue = m_AccountsDBAide.ExecuteProcess(TEXT("GSP_GR_LOAD_GAME_CONFIG_TUI_GUANG"), true);
		CMD_GP_PopularizeHttpRes popularizeHttpRes;
		if (lReturnValue == DB_SUCCESS) {
			popularizeHttpRes.dwVersion = m_AccountsDBAide.GetValue_INT(TEXT("Version"));
			m_AccountsDBAide.GetValue_String(TEXT("DownLoadTuiGuangHttp"), popularizeHttpRes.szPopularizeHttp, CountArray(popularizeHttpRes.szPopularizeHttp));
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GP_POPULARIZE_HTTP, dwContextID, &popularizeHttpRes, sizeof(popularizeHttpRes));
		}
		return true;
	} catch (IDataBaseException * pIException) {
		//错误信息
...
bool CDataBaseEngineSink::OnRequestGameRecordListEx(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	try
	{
		//请求处理
...
bool CDataBaseEngineSink::OnRequestGameTotalRecord(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	try
	{
		...
		tagPrivateRandTotalRecord kTotalRecord;
		kTotalRecord.iRecordID = pNetInfo->dwRecordID;
...
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient)
{
	if (bMobileClient==false)
	{
		if (dwErrorCode==DB_SUCCESS)
		{
			...
			LogonSuccess.cbInsureEnabled= static_cast<BYTE>(m_AccountsDBAide.GetValue_DWORD(TEXT("InsureEnabled")));
