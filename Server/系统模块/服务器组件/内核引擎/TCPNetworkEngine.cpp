...
#define TIME_BREAK_READY			240000L								//中断时间
#define TIME_BREAK_CONNECT			120000L								//中断时间
#define TIME_DETECT_SOCKET			480000L								//监测时间
...
//设置参数
bool  CTCPNetworkEngine::SetServiceParameter(WORD wServicePort, WORD wMaxConnect,const TCHAR*)
{
	//效验状态
	if (m_bService == true)
	{
...
//批量发送
bool  CTCPNetworkEngine::SendDataBatch(WORD wMainCmdID, WORD wSubCmdID, void * pData, WORD wDataSize,LONG)
{
	//效益状态
	if (m_bService == false) return false;
...
//允许群发
bool  CTCPNetworkEngine::AllowBatchSend(DWORD dwSocketID, bool bAllow,LONG)
{
