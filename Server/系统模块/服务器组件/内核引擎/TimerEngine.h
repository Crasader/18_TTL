...
public:
	//设置定时器
	virtual bool  SetTimer(DWORD dwTimerID, DWORD dwElapse, DWORD dwRepeat, WPARAM dwBindParameter) override;
	//删除定时器
	virtual bool  KillTimer(DWORD dwTimerID) override;
	//删除定时器
	virtual bool  KillAllTimer() override;
	//设置接口
	virtual bool  SetTimerEngineEvent(IUnknownEx * pIUnknownEx) override;
