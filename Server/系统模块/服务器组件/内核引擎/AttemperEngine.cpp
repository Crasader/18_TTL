	...
	switch (wIdentifier)
	{
		...
		case EVENT_TCP_NETWORK_READ:	//网络读取事件
		{
			...
			if (bSuccess == false)
			{
				...
				TCHAR kTemp[256];
				sprintf_s(kTemp,"if (bSuccess == false) OnEventTCPNetworkRead %d %d",pSocketReadEvent->Command.wMainCmdID,pSocketReadEvent->Command.wSubCmdID);
				CTraceService::TraceString(kTemp,TraceLevel_Exception);
			}
		...
		case EVENT_TCP_SOCKET_READ:		//网络读取事件
		{
			...
			if (bSuccess == false && m_pITCPSocketService)
...
//////////////////////////////////////////////////////////////////////////
//应答事件
bool  CAttemperEngine::OnEventTCPNetworkBind(DWORD dwSocketID, DWORD dwClientIP)
{
	//缓冲锁定
...
//关闭事件
bool  CAttemperEngine::OnEventTCPNetworkShut(DWORD dwSocketID, DWORD dwClientIP, DWORD dwActiveTime)
{
	//缓冲锁定
	CWHDataLocker lock(m_CriticalSection);
