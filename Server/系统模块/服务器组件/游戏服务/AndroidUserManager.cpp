	...
	case IDI_ANDROID_INOUT:			//进出处理
		{
			...
			while (Position!=NULL)
			{
				...
				if (pAndroidUserItem->m_pIServerUserItem!=NULL)
				{
					...
					if ((cbUserStatus != US_FREE) && (cbUserStatus != US_FREE)) {
						if ((pAndroidUserItem->GetAndroidParameter()->dwServiceTime&dwTimeMask) == 0L && (cbUserStatus == US_SIT || cbUserStatus == US_READY)) {
							CMD_GR_UserStandUp UserStandUp;
							ZeroMemory(&UserStandUp, sizeof(UserStandUp));
							UserStandUp.wTableID = pIServerUserItem->GetTableID();
							UserStandUp.wChairID = pIServerUserItem->GetChairID();
							UserStandUp.cbForceLeave = true;
							//发送数据
							SendDataToServer(MAKELONG(pAndroidUserItem->m_wAndroidIndex, pAndroidUserItem->m_wRoundID), MDM_GR_USER, SUB_GR_USER_STANDUP, &UserStandUp, sizeof(UserStandUp));
						}
						continue;
					}
	...
	case IDI_ANDROID_PULSE:		//用户脉冲
		{
			...
			CAndroidUserItem * pAndroidUserItem=NULL;
			POSITION Position=m_AndroidUserItemMap.GetStartPosition();
			std::vector<CAndroidUserItem *> kVectorAndroidUserItem;
			//用户处理
			while (Position!=NULL)
			{
				//获取对象
				m_AndroidUserItemMap.GetNextAssoc(Position,dwUserID,pAndroidUserItem);
				kVectorAndroidUserItem.push_back(pAndroidUserItem);
			}
			//时间处理
			for (std::vector<CAndroidUserItem *>::iterator itor = kVectorAndroidUserItem.begin();
				itor != kVectorAndroidUserItem.end();itor++)
			{
				try
				{
					pAndroidUserItem=*itor;
