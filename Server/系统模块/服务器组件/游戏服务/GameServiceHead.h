...
#define GER_MASTER_END					0x04								//进程控制，游戏结束
...
struct tagTableFrameParameter
{
	...
	IUnknownEx *					pIGamePrivateServiceManager;		//私人场管理
};
...
{
	//用户信息
	DWORD							dwRecordID;							//用户标识

	//成绩信息
	SCORE							lScore;								//用户分数
	SCORE							lGrade;								//用户成绩

	SYSTEMTIME						PlayerTime;							//游戏结束时间
...
public:
    ...
    //设置用户积分
    virtual void SetUserScore(SCORE score) = NULL;
	//设置用户房卡数量
	virtual void SetUserInsureScore(SCORE InsureScore) = NULL;
	//用户成绩
...
public:
	...
	virtual void SetTableInfoExtra(void* tableInfo) = NULL;

	virtual void* GetTableInfoExtra() = NULL;

	virtual void RemoveTableInfoExtra() = NULL;
...
public:
	...
	//起立动作
	virtual bool PerformStandUpActionReally(IServerUserItem * pIServerUserItem, BOOL bReserveRoom = false, INT  nForceUserStatus = US_ERROR )=NULL;
	//旁观动作
