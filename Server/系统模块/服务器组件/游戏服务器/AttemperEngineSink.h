...
protected:
	...
	ITCPSocketService *				m_pITCPSocketServicePayment;		//支付网络服务
...
protected:
    //注册事件
    bool OnTCPSocketPaymentMainRegister(WORD wSubCmdID, VOID * pData, WORD wDataSize);
    //订单事件
    bool OnTCPSocketPaymentMainBusiness(WORD wSubCmdID, VOID * pData, WORD wDataSize);

	//网络事件
protected:
    //支付命令
    bool OnTCPNetworkMainPay(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID);
	//用户处理
    ...
    //系统消息
    bool OnTCPNetworkSystemMessage(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID);
