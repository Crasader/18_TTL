...
bool CServiceUnits::ConcludeService()
{
	...
	if (m_TCPSocketServicePayment.GetInterface()!=NULL) m_TCPSocketServicePayment->ConcludeService();
#endif
...
bool CServiceUnits::CreateServiceDLL()
{
    ...
    //支付网络组件
#ifdef ALLOWED_CONECT_PAYMENT
    if ((m_TCPSocketServicePayment.GetInterface()==NULL)&&(m_TCPSocketServicePayment.CreateInstance()==false))
    {
        CTraceService::TraceString(m_TCPSocketServicePayment.GetErrorDescribe(),TraceLevel_Exception);
        return false;
    }
#endif
...
#ifdef ALLOWED_CONECT_PAYMENT
    //支付服务
    if (m_TCPSocketServicePayment->SetServiceID(NETWORK_PAYMENT)==false) return false;
    if (m_TCPSocketServicePayment->SetTCPSocketEvent(pIAttemperEngine)==false) return false;
#endif
...
#ifdef ALLOWED_CONECT_PAYMENT
    m_AttemperEngineSink.m_pITCPSocketServicePayment=m_TCPSocketServicePayment.GetInterface();
#endif
...
#ifdef ALLOWED_CONECT_PAYMENT
    //支付引擎
    if (m_TCPSocketServicePayment->StartService()==false)
    {
        ASSERT(FALSE);
        return false;
    }
	...
	case UI_CORRESPOND_RESULT:		//协调成功
		{
            ...
            else if(m_ServiceStatus==ServiceStatus_Service)
            {
                CTraceService::TraceString(TEXT("服务已经启动"),TraceLevel_Normal);
            }
...
#ifdef ALLOWED_CONECT_PAYMENT
                //连接支付
                SendControlPacket(CT_CONNECT_PAYMENT_SERVER,NULL,0);
#endif
