...
#define IDI_CONNECT_PAYMENT			(IDI_MAIN_MODULE_START+11)			//连接时间
...
CAttemperEngineSink::CAttemperEngineSink()
{
	...
	m_pIPrivateServiceManager = NULL;
...
bool CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	else if(m_pIPrivateServiceManager != NULL)
	{
		if (m_ServerUserManager.SetServerUserItemSink(m_pIPrivateServiceManager->GetServerUserItemSink())==false)
		{
			ASSERT(FALSE);
			return false;
		}
...
bool CAttemperEngineSink::OnAttemperEngineConclude(IUnknownEx * pIUnknownEx)
{
	...
	if(m_pIPrivateServiceManager!=NULL)
		m_pIPrivateServiceManager->StopService();
...
#ifdef  ALLOWED_CONECT_PAYMENT
    case CT_CONNECT_PAYMENT_SERVER:		//连接支付
        {
            //发起连接
            tagAddressInfo * pPaymentAddress=&m_pInitParameter->m_PaymentAddress;
            m_pITCPSocketServicePayment->Connect(pPaymentAddress->szAddress,m_pInitParameter->m_wPaymentPort);

            //构造提示
            TCHAR szString[512]=TEXT("");
            _sntprintf(szString,CountArray(szString),TEXT("正在连接支付服务器 [ %s:%d ]"),pPaymentAddress->szAddress,m_pInitParameter->m_wPaymentPort);
...
bool CAttemperEngineSink::OnEventTimer(DWORD dwTimerID, WPARAM wBindParam)
{
	...
	if ((dwTimerID>=IDI_MAIN_MODULE_START)&&(dwTimerID<=IDI_MAIN_MODULE_FINISH))
	{
		...
		case IDI_REPORT_SERVER_INFO:	//房间信息
			{
                //if( CServiceUnits::g_pServiceUnits && CServiceUnits::g_pServiceUnits->GetServiceStatus() == ServiceStatus_Service)
                //{
							...
							for (INT_PTR i=0;i<m_pGameServiceOption->wTableCount;i++)
							{
								...
								for (WORD j=0;j<AndroidInitiative.wFreeUserCount;j++)
								{
									...
									case NN_GAME_KIND_ID: {
										...
										if (pIAndroidUserItem->GetMeUserItem()->GetUserScore() < tableInfo->dwEnterMatchNum) {
											continue;
										}
									}
										break;
							...
							if (m_pGameServiceOption->wServerType == GAME_GENRE_EDUCATE) {
								if (m_pGameServiceOption->wKindID == NN_GAME_KIND_ID) {
									...
									for (WORD j = 0; j < AndroidInitiative.wFreeUserCount; j++)
									{
										...
										DWORD gameRuleIndex = 0;
										FvMask::Add(gameRuleIndex, _MASK_(1));
										FvMask::Add(gameRuleIndex, _MASK_(15));
										FvMask::Add(gameRuleIndex, _MASK_(16));
										FvMask::Add(gameRuleIndex, _MASK_(17));
										...
										createRoom.bGameRuleIdex = gameRuleIndex;
										createRoom.bAllowedStrangerJoin = true;
										createRoom.dwEnterMatchNum = pIAndroidUserItem->GetAndroidParameter()->lMinTakeScore * 40;
										createRoom.dwBaseScore = pIAndroidUserItem->GetAndroidParameter()->lMinTakeScore;
										createRoom.dwOutMatchNum = pIAndroidUserItem->GetAndroidParameter()->lMinTakeScore * 30;
										createRoom.wCharNum = -1;

										if (pIAndroidUserItem->GetMeUserItem()->GetUserScore() < createRoom.dwEnterMatchNum) {
											continue;
										}

										OnTCPNetworkMainPrivate(SUB_GR_CREATE_PRIVATE, &createRoom, sizeof(createRoom), pIAndroidUserItem->GetMeUserItem()->GetBindIndex());
										return true;
									}
								}
							}
	...
	//比赛定时器
	if((dwTimerID>=IDI_PRIVATE_MODULE_START)&&(dwTimerID<IDI_PRIVATE_MODULE_FINISH))
	{
		if(m_pIPrivateServiceManager!=NULL) m_pIPrivateServiceManager->OnEventTimer(dwTimerID,wBindParam);
		return true;
	}
...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	//比赛事件
	if(wRequestID>=DBO_GR_PRIVATE_EVENT_START && wRequestID<=DBO_GR_PRIVATE_EVENT_END)
	{
		//参数效验
		if(m_pIPrivateServiceManager==NULL) return false;
		...
		if (wRequestID == DBO_GR_PRIVAT_REFRESH_TABLES_END) 
		{
			...
			DBR_GP_Private_Refresh_Table_Ret* pRet = static_cast<DBR_GP_Private_Refresh_Table_Ret*>(pData);
			pBindParameter = GetBindParameter(LOWORD(pRet->dwUserSocketID));
			pIServerUserItem = (pBindParameter != NULL ? pBindParameter->pIServerUserItem : NULL);
		}
		return m_pIPrivateServiceManager->OnEventDataBase(wRequestID,pIServerUserItem,pData,wDataSize);
	}
...
bool CAttemperEngineSink::OnEventTCPSocketShut(WORD wServiceID, BYTE cbShutReason)
{
    switch(wServiceID)
    {
        ...
        case NETWORK_PAYMENT:
        {
            //构造提示
            TCHAR szDescribe[128]=TEXT("");
            _sntprintf(szDescribe,CountArray(szDescribe),TEXT("与支付服务器的连接关闭了，%ld 秒后将重新连接"),m_pInitParameter->m_wConnectTime);
            ...
            //设置时间
            ASSERT(m_pITimerEngine!=NULL);
            m_pITimerEngine->SetTimer(IDI_CONNECT_PAYMENT,m_pInitParameter->m_wConnectTime*1000L,1,0);
    ...
    switch(wServiceID)
    {
        ...
        case NETWORK_PAYMENT:
        {
            ...
            if (nErrorCode!=0)
            {
                //构造提示
                TCHAR szDescribe[128]=TEXT("");
                _sntprintf(szDescribe,CountArray(szDescribe),TEXT("支付服务器连接失败 [ %ld ]，%ld 秒后将重新连接"),
                    nErrorCode,m_pInitParameter->m_wConnectTime);
            ...
            //提示消息
            CTraceService::TraceString(TEXT("正在发送支付服务器注册信息..."),TraceLevel_Normal);

            CServiceUnits * pServiceUnits=CServiceUnits::g_pServiceUnits;

            CMD_PM_Regist pm_regist;
            pm_regist.wServerKindID = pServiceUnits->m_GameServiceOption.wKindID;
            pm_regist.wServerID = pServiceUnits->m_GameServiceOption.wServerID;
            memcpy(pm_regist.csPlatformID, m_pInitParameter->m_PaymentID.szAddress, sizeof(m_pInitParameter->m_PaymentID));

            ASSERT(m_pITCPSocketServicePayment!=NULL);
            m_pITCPSocketServicePayment->SendData(MDM_PM_REGIST, SUB_PM_REGIST, &pm_regist, sizeof(pm_regist));
	...
	switch (wServiceID)
	{
        case NETWORK_CORRESPOND:
        {
            switch (Command.wMainCmdID)
            {
                ...
                case MDM_C2G_SYSTEM_MSG:
                {
                    return OnTCPNetworkSystemMessage(Command.wMainCmdID, Command.wSubCmdID, pData, wDataSize, 0);
                }
            }
            break;
        }
        ...
        case NETWORK_PAYMENT:
        {
            switch(Command.wMainCmdID)
            {
                case MDM_PM_REGIST:
                {
                    return OnTCPSocketPaymentMainRegister(Command.wSubCmdID,pData,wDataSize);
                }
                case MDM_PM_BESINESS:
                {
                    return OnTCPSocketPaymentMainBusiness(Command.wSubCmdID,pData,wDataSize);
                }
            }
            break;
        }
...
bool CAttemperEngineSink::OnEventTCPNetworkRead(TCP_Command Command, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case MDM_GR_PRIVATE:		//比赛命令
		{
			return OnTCPNetworkMainPrivate(Command.wSubCmdID,pData,wDataSize,dwSocketID);
		}
    case MDM_GF_PAY:		//支付命令
        {
            return OnTCPNetworkMainPay(Command.wSubCmdID, pData, wDataSize, dwSocketID);
        }
	}
...
bool CAttemperEngineSink::OnEventUserItemScore(IServerUserItem * pIServerUserItem, BYTE cbReason)
{
	...
	//投递请求
	DBR_GR_Beginner_InGame kNetBeginnerInfo;
	kNetBeginnerInfo.dwUserID = pIServerUserItem->GetUserID();
	m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(),DBR_GR_BEGINNER_GAME_TIME,0L,&kNetBeginnerInfo,sizeof(kNetBeginnerInfo), TRUE);
...
bool CAttemperEngineSink::OnEventUserItemStatus(IServerUserItem * pIServerUserItem, WORD wOldTableID, WORD wOldChairID)
{
    ...
    WORD UserTableID = pIServerUserItem->GetTableID();
	...
	UserStatus.UserStatus.wTableID= UserTableID;
	...
	if(pIServerUserItem->GetUserStatus()==US_SIT && pIServerUserItem->IsMobileUser())
	{
		...
		if(UserTableID > (wTagerDeskPos+wTagerDeskCount-1))
		{
			//定义变量
			WORD wNewDeskPos = (UserTableID/wTagerDeskCount)*wTagerDeskCount;
	...
	if (pIServerUserItem->GetUserStatus()==US_READY && 
		((m_pGameServiceOption->wServerType&GAME_GENRE_MATCH)!=0 || (m_pGameServiceOption->wServerType&GAME_GENRE_EDUCATE)!=0))
	{
		//状态校验
	...
	case SUB_CS_S_SERVER_INFO:		//房间信息
		{
			...
			if (m_pGameServiceOption->wServerType==GAME_GENRE_MATCH)
			{
				...
				RegitsterMatch.lMatchEnterScore =  m_pGameMatchOption->lMatchEnterScore;
...
bool CAttemperEngineSink::OnTCPSocketPaymentMainRegister(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
    switch (wSubCmdID)
    {
    case SUB_PM_REGIST_RET:	//系统消息
        {
            ASSERT(wDataSize == sizeof(CMD_PM_Regist_Result));
            CMD_PM_Regist_Result* reg_result = static_cast<CMD_PM_Regist_Result*>(pData);
            
            //提示消息
            if(reg_result->wRegistResult != 0){

                TCHAR infoText[256] = {0};
                sprintf_s(infoText, "注册支付服务失败, error code = %d", reg_result->wRegistResult);
                CTraceService::TraceString(infoText, TraceLevel_Exception);

            } else {

                TCHAR infoText[256] = { 0 };
                sprintf_s(infoText, "支付服务注册成功.");
                CTraceService::TraceString(infoText, TraceLevel_Normal);
...
bool CAttemperEngineSink::OnTCPSocketPaymentMainBusiness(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
    switch (wSubCmdID)
    {
        case SUB_PM_PAY_WEIXINORDER_RET:
        {
            //返回订单结果
            ...
            //获取信息
            WORD wBindIndex = LOWORD(odRet->dwSocketID);
            IServerUserItem * pIServerUserItem = GetBindUserItem(wBindIndex);

            return CServiceUnits::g_pServiceUnits->m_PrivateServiceManager->OnEventSocketPrivate(SUB_GR_PAYMENT_WEIXIN_ORDER_RET, pData, wDataSize, pIServerUserItem, 0L);
        }
        case SUB_PM_PAY_SUCCESS:	//支付成功
        {
            //刷新桌子数据
            return CServiceUnits::g_pServiceUnits->m_PrivateServiceManager->OnEventSocketPrivate( SUB_GR_PAYMENT_REFRESH_TABLE, pData, wDataSize, NULL, 0L);
        }
    }
...
//比赛命令
bool CAttemperEngineSink::OnTCPNetworkMainPrivate(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//获取信息
	WORD wBindIndex=LOWORD(dwSocketID);
	IServerUserItem * pIServerUserItem=GetBindUserItem(wBindIndex);
	...
	//效验接口
	ASSERT(m_pIPrivateServiceManager!=NULL);
	if (m_pIPrivateServiceManager==NULL) return false;

	//消息处理
	return m_pIPrivateServiceManager->OnEventSocketPrivate(wSubCmdID,pData,wDataSize,pIServerUserItem,dwSocketID);
}

bool CAttemperEngineSink::OnTCPNetworkMainPay(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
    //获取用户
    WORD wBindIndex = LOWORD(dwSocketID);
    IServerUserItem * pIServerUserItem = GetBindUserItem(wBindIndex);
    ...
    ASSERT(wDataSize == sizeof(CMD_GF_Pay_GenWXOrder));
    if (wDataSize != sizeof(CMD_GF_Pay_GenWXOrder)) return false;

    CMD_GF_Pay_GenWXOrder* pGenOrder = static_cast<CMD_GF_Pay_GenWXOrder*>(pData);

    SUB_PM_GEN_WEIXIN_OD weixin_order;

    memset(&weixin_order, 0, sizeof(weixin_order));
    memcpy(weixin_order.cstrIP, pGenOrder->cstrIP,sizeof(pGenOrder->cstrIP));
    memcpy(weixin_order.cstrAttach, pGenOrder->cstrAttach, (pGenOrder->wAttachLen > ATTACH_LEN ? ATTACH_LEN : pGenOrder->wAttachLen));

    weixin_order.dwTableID = pIServerUserItem->GetTableID();
    weixin_order.dwUserID = pIServerUserItem->GetUserID();
    weixin_order.dwSocketID = dwSocketID;
    weixin_order.wPoductID = pGenOrder->wProductID;

    m_pITCPSocketServicePayment->SendData(MDM_PM_BESINESS, SUB_PM_PAY_GEN_WEIXINORDER, &weixin_order, sizeof(weixin_order));

    return true;
}

bool CAttemperEngineSink::OnTCPNetworkSystemMessage(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
    return m_pITCPNetworkEngine->SendDataBatch(wMainCmdID, wSubCmdID, pData, wDataSize, 0L);
}
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonUserID(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//私人场效验
	if(m_pIPrivateServiceManager!=NULL&&m_pIPrivateServiceManager->OnEventEnterPrivate(dwSocketID, pData, pBindParameter->dwClientAddr,false))
	{
		//SendRoomMessage(dwSocketID, szPrint, SMT_CHAT|SMT_EJECT|SMT_GLOBAL|SMT_CLOSE_ROOM,(pBindParameter->dwClientAddr==0L));
		return true;
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonMobile(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//私人场效验
	if(m_pIPrivateServiceManager!=NULL&&m_pIPrivateServiceManager->OnEventEnterPrivate(dwSocketID, pData, pBindParameter->dwClientAddr,true))
	{
		return true;
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubUserSitDown(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//插入分组
	if (m_pIMatchServiceManager)
	{
		CMD_GR_Match_JoinResoult kResoult;
		kResoult.wSucess = InsertDistribute(pIServerUserItem);
		SendData(pIServerUserItem,MDM_GR_MATCH,SUB_GR_MATCH_JOIN_RESOULT,&kResoult,sizeof(kResoult));
		return true;
	}

	//插入分组
	if (m_pIPrivateServiceManager)
	{
...
bool CAttemperEngineSink::OnTCPNetworkSubUserStandUp(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	{
		//掉线通知
		if(m_pIPrivateServiceManager!=NULL) m_pIPrivateServiceManager->OnEventReqStandUP(pIServerUserItem);
...
VOID CAttemperEngineSink::OnEventUserLogon(IServerUserItem * pIServerUserItem, bool bAlreadyOnLine)
{
	...
	//登录通知
	if(m_pIPrivateServiceManager!=NULL) m_pIPrivateServiceManager->OnEventUserLogon(pIServerUserItem);
...
VOID CAttemperEngineSink::OnEventUserLogout(IServerUserItem * pIServerUserItem, DWORD dwLeaveReason)
{
	...
	if(m_pIPrivateServiceManager!=NULL) 
	{
		m_pIPrivateServiceManager->OnEventUserQuitPrivate(pIServerUserItem, 0);
	}
	...
	//登出通知
	if(m_pIPrivateServiceManager!=NULL) m_pIPrivateServiceManager->OnEventUserLogout(pIServerUserItem);
...
bool CAttemperEngineSink::PerformDistribute()
{
	...
	{
		//人数校验
		if(m_DistributeManage.GetCount()<m_pGameServiceOption->wMinDistributeUser) break;

		//变量定义
		CDistributeInfoArray DistributeInfoArray;

		//获取用户
		WORD wRandCount = __max(m_pGameServiceOption->wMaxDistributeUser-m_pGameServiceOption->wMinDistributeUser+1,1);
		WORD wChairCount = m_pGameServiceOption->wMinDistributeUser+rand()%wRandCount;
...
bool CAttemperEngineSink::PerformCheckVersion(DWORD dwPlazaVersion, DWORD dwFrameVersion, DWORD dwClientVersion, DWORD dwSocketID)
{
	...
	//	lstrcpyn(UpdateNotify.szDownLoadHttp,m_pGameServiceAttrib->szClientEXEName,CountArray(m_pGameServiceAttrib->szClientEXEName));
	//	
	//	//发送消息
...
bool CAttemperEngineSink::SendRequestFailure(IServerUserItem * pIServerUserItem, LPCTSTR pszDescribe, LONG lErrorCode)
{
	...
	SendData(pIServerUserItem,MDM_GR_USER,SUB_GR_SIT_FAILED,&RequestFailure,wHeadSize+wDataSize);
...
bool CAttemperEngineSink::SendUserInfoPacket(IServerUserItem * pIServerUserItem, IServerUserItem * pISendToUserItem)
{
	WORD wBindIndex=pISendToUserItem->GetBindIndex();
	tagBindParameter * pBindParameter=GetBindParameter(wBindIndex);
	if (pBindParameter && pBindParameter->dwSocketID != INVALID_DWORD)
	{
		SendUserInfoPacket(pIServerUserItem,pBindParameter->dwSocketID);
	}
	return true;
...
bool CAttemperEngineSink::InitTableFrameArray()
{
	...
	if(m_pIMatchServiceManager!=NULL)
		TableFrameParameter.pIGameMatchServiceManager=QUERY_OBJECT_PTR_INTERFACE(m_pIMatchServiceManager,IUnknownEx);

	if(m_pIPrivateServiceManager!=NULL)
		TableFrameParameter.pIGamePrivateServiceManager=QUERY_OBJECT_PTR_INTERFACE(m_pIPrivateServiceManager,IUnknownEx);
	...
	for (WORD i=0;i<m_pGameServiceOption->wTableCount;i++)
	{
		...
		if(m_pIPrivateServiceManager!=NULL)
		{
			m_pIPrivateServiceManager->BindTableFrame(QUERY_OBJECT_PTR_INTERFACE((m_TableFrameArray[i]),ITableFrame),i);
		}
	}
...
bool CAttemperEngineSink::InitPrivateServiceManager()
{
	//参数校验
	if(m_pIPrivateServiceManager==NULL) return true;

	//桌子参数
	tagPrivateManagerParameter kPrivateManagerParameter;
	ZeroMemory(&kPrivateManagerParameter,sizeof(kPrivateManagerParameter));

	//配置参数
	kPrivateManagerParameter.pGameServiceOption=m_pGameServiceOption;
	kPrivateManagerParameter.pGameServiceAttrib=m_pGameServiceAttrib;

	kPrivateManagerParameter.m_pIGameServiceManager = m_pIGameServiceManager;

	//内核组件
	kPrivateManagerParameter.pITimerEngine=m_pITimerEngine;
	kPrivateManagerParameter.pICorrespondManager=m_pIDBCorrespondManager;
	kPrivateManagerParameter.pTCPNetworkEngine=QUERY_OBJECT_PTR_INTERFACE(m_pIAttemperEngine,ITCPNetworkEngineEvent);

	//服务组件
	kPrivateManagerParameter.pIMainServiceFrame=this;
	kPrivateManagerParameter.pIServerUserItemSink=this;
	kPrivateManagerParameter.pIAndroidUserManager=&m_AndroidUserManager;
	kPrivateManagerParameter.pIServerUserManager=QUERY_OBJECT_INTERFACE(m_ServerUserManager,IServerUserManager);		

	//初始化接口
	m_pIPrivateServiceManager->InitPrivateInterface(kPrivateManagerParameter);
...
bool CAttemperEngineSink::InsertDistribute(IServerUserItem * pIServerUserItem)
{
	...
	if(m_pIPrivateServiceManager!=NULL) 
	{
		...
		//加入比赛
		return m_pIPrivateServiceManager->OnEventUserJoinPrivate(pIServerUserItem,0,pBindParameter->dwSocketID);
	}
