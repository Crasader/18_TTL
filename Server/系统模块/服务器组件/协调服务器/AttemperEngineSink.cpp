...
bool CAttemperEngineSink::loadMsgText()
{
    //获取路径
    ...
    //构造路径
    TCHAR zsMsgFilepath[MAX_PATH] = TEXT("");
    _sntprintf(zsMsgFilepath, CountArray(zsMsgFilepath), TEXT("%s\\%s"), szWorkDir, m_pInitParameter->m_szMsgFileName);

    if ( !PathFileExists(zsMsgFilepath) && !PathFileExists(m_pInitParameter->m_szMsgFileName) )
    {
        return false;
    }

    m_SysMsg.nFrequency = GetPrivateProfileInt(TEXT("MessageInfo"), TEXT("MessageFrequency"), -2, zsMsgFilepath);
    m_SysMsg.wSendTime = GetPrivateProfileInt(TEXT("MessageInfo"), TEXT("SendTime"), -2, zsMsgFilepath);

    GetPrivateProfileString(TEXT("MessageInfo"), TEXT("MessageText"), "", m_SysMsg.szSysMsg, SYS_MSG_LENGTH, zsMsgFilepath);

    if (m_SysMsg.nFrequency == -2 || m_SysMsg.wSendTime == -2 || m_SysMsg.szSysMsg[0] == 0)
    {
        return false;
    }

    //构造路径
    TCHAR zsReadMsgFilepath[MAX_PATH] = TEXT("");
    _sntprintf(zsReadMsgFilepath, CountArray(zsReadMsgFilepath), TEXT("%s\\%s.readover"), szWorkDir, m_pInitParameter->m_szMsgFileName);

    DeleteFile(zsReadMsgFilepath);
    CFile::Rename(zsMsgFilepath, zsReadMsgFilepath);

    return true;
}

bool CAttemperEngineSink::SendSystemMessage(TCHAR *szInfo)
{
    CMD_MB_System_Notify_C data;
    ...
    //do
    //{
    //    CGlobalPlazaItem *p = m_GlobalInfoManager.EnumPlazaItem(position);
    //    if (p == NULL)
    //    {
    //        return true;
    //    }
    //    m_pITCPNetworkEngine->SendData(p->m_GamePlaza.dwSocket, MDM_C2G_SYSTEM_MSG, SUB_C2L_SYSTEM_NOTIFY, &data, sizeof(data));

    //} while (position != NULL);

    m_pITCPNetworkEngine->SendDataBatch(MDM_C2G_SYSTEM_MSG, SUB_C2L_SYSTEM_NOTIFY, &data, data.size(), 0L);
...
bool CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx * pIUnknownEx)
{
    ...
    m_pITimerEngine->SetTimer(IDI_LOAD_SYSTEM_MESSAGE, SYSTEM_MESSAGE_DETECT_TIME, TIMES_INFINITY, 0);
...
bool CAttemperEngineSink::OnEventTimer(DWORD dwTimerID, WPARAM wBindParam)
{
    ...
    switch (dwTimerID) {
        ...
        case IDI_LOAD_SYSTEM_MESSAGE:
        {
            ...
            if (loadMsgText()) 
            {
                m_pITimerEngine->KillTimer(IDI_SEND_SYSTEM_MESSAGE);
                m_pITimerEngine->SetTimer(IDI_SEND_SYSTEM_MESSAGE, m_SysMsg.wSendTime, m_SysMsg.nFrequency, 0);
            }
        ...
        case IDI_SEND_SYSTEM_MESSAGE:
        {
            ...
            SendSystemMessage(m_SysMsg.szSysMsg);
