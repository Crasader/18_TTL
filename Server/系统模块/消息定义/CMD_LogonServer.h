...
struct CMD_GP_LogonSuccess
{
	...
	TCHAR							szGroupName[LEN_GROUP_NAME*2];		//社团名字
...
#define SUB_GP_POPULARIZE_HTTP 562 //绑定推荐码
...
#define  SUB_GP_SHOPCARD					1103					//购买房卡
...
enum PopularizeType {
	PopularizeType_0, //大赢家
	PopularizeType_1, //土豪
};

//推荐人列表
struct CMD_GP_PopularizeList
{
	BYTE bPopularizeType; // 类型-PopularizeType
	DWORD dwGameKindID; // 游戏ID
};

struct CMD_GP_PopularizeInfo
{
	INT iPopularizeID; // 推广员id
	INT iRankCount; //上榜次数
	DWORD dwUserID; // 推广员uid
	LONGLONG lScore; //金币数
	TCHAR szPopularizeCode[LEN_POPULARIZE_CODE]; //邀请码
	TCHAR szName[LEN_NICKNAME]; // 昵称
	TCHAR szAvatar[LEN_USER_NOTE]; //头像

	void StreamValue(datastream& kData, bool bSend)
	{
		Stream_VALUE(iPopularizeID);
		Stream_VALUE(iRankCount);
		Stream_VALUE(dwUserID);
		Stream_VALUE(lScore);
		Stream_VALUE_SIZE(szPopularizeCode, sizeof(szPopularizeCode));
		Stream_VALUE_SIZE(szName, sizeof(szName));
		Stream_VALUE_SIZE(szAvatar, sizeof(szAvatar));
	}
};
...
struct CMD_GP_PopularizeListRes
{
	BYTE bPopularizeType; // 类型-PopularizeType
	DWORD dwGameKindID; // 游戏ID
	std::vector<CMD_GP_PopularizeInfo> vcPopularizeList; //推广员列表

	void StreamValue(datastream& kData, bool bSend)
	{
		Stream_VALUE(bPopularizeType);
		Stream_VALUE(dwGameKindID);
		StructVecotrMember(CMD_GP_PopularizeInfo, vcPopularizeList);
	}
};

enum PopularizeBindResult {
	PopularizeBindResult_0, //绑定成功
	PopularizeBindResult_1, //绑定失败-已绑定
	PopularizeBindResult_2, //绑定失败-不存在的邀请码
};

struct CMD_GP_PopularizeBind
{
	DWORD dwOperatorUser; //绑定人
	TCHAR szPopularizeCode[LEN_POPULARIZE_CODE]; //邀请码
};

struct CMD_GP_PopularizeBindRes
{
	BYTE bResult; //绑定结果-PopularizeBindResult
};

struct CMD_GP_PopularizeHttp
{
	DWORD dwVersion; //版本
};
...
{
	DWORD dwVersion; //版本
	TCHAR szPopularizeHttp[128]; //http 图片url
};
