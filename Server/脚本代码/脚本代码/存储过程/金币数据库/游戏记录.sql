...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_GP_PrivateGameRecord_Child]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_GP_PrivateGameRecord_AllChild]
GO
...
-- 执行逻辑
BEGIN
	SELECT * FROM PrivateGameRecordChild WHERE RecordChildID=@dwRecordID
END
...
----------------------------------------------------------------------------------------------------
-- 游戏记录
CREATE PROC GSP_GP_PrivateGameRecord_AllChild
	@dwRecordID INT								
WITH ENCRYPTION AS
