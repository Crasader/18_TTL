...
BEGIN
	...
	DECLARE @RegisterDate AS DATETIME
	SELECT @UserID=UserID, @GameID=GameID, @Accounts=Accounts, @NickName=NickName, @UnderWrite=UnderWrite, @LogonPass=LogonPass,@strInsurePass=InsurePass,
		...
		@MoorMachine=MoorMachine,@SpreaderID=SpreaderID,@PlayTimeCount=PlayTimeCount,@strLogonIP=LastLogonIP,@RegisterDate=RegisterDate
	...
	SELECT @UserID AS UserID, @GameID AS GameID, @Accounts AS Accounts, @NickName AS NickName, @UnderWrite AS UnderWrite,
		...
		@MoorMachine AS MoorMachine,@SpreaderID AS SpreaderID,@strLogonIP AS LogonIP, @RegisterDate AS RegisterDate
...
BEGIN
	...
	SELECT @dwUserID AS UserID, @Compellation AS Compellation, @QQ AS QQ, @EMail AS EMail, @SeatPhone AS SeatPhone,
		@MobilePhone AS MobilePhone, @DwellingPlace AS DwellingPlace, @UserNote AS UserNote, @HeadHttp AS HeadHttp, @UserChannel AS UserChannel, @UserIP AS UserIP
